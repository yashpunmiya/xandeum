...
export async function updateNodes(network: 'mainnet' | 'devnet' = 'devnet') {
    try {
        ...
        const processNode = async (rawNode: any) => {
            ...
            // 4. SCORING ALGORITHM (v2 - "Fair Rank")

            // A. Reliability Score (Non-Linear / Sqrt Curve)
            // Motivation: A linear 7-day target unfairly punishes new high-performance nodes.
            // A sqrt curve allows nodes to reach ~53% score in 2 days, while still requiring 7 days for 100%.
            const uptimeDays = (uptime || 0) / 86400.0;
            let s_reliability = 0;
            if (uptimeDays > 0) {
                // Curve: y = sqrt(x/7) * 100
                s_reliability = Math.min(100, Math.sqrt(uptimeDays / 7.0) * 100);
            }
            // Fallback: Active via RPC but stats hidden/zero uptime
            if (s_reliability === 0 && rpc_active) {
                s_reliability = 25.0; // Bumped slightly
            }

            // B. Performance Score (RAM + Storage)
            const GB_DIVISOR = 1_000_000_000.0;

            // RAM Target: 64GB (Strict)
            ...
            if (geo.country && geo.country !== 'Unknown' && totalNodesWithCountry > 0) {
                ...
                // Penalize only high concentration > 30%
                if (concentration >= 0.30) s_decentralization = 50.0;
                if (concentration >= 0.50) s_decentralization = 0.0;
            }

            // D. Version Score (Strict)
            // v1.x is the new standard. v0.x is legacy.
            let s_version = 0;
            const vParts = version.split('.').map(Number);
            if (vParts.length >= 2) {
                if (vParts[0] >= 1) s_version = 100.0; // v1.x.x
                else if (vParts[0] === 0 && vParts[1] >= 8) s_version = 60.0; // v0.8+ (Legacy)
                else s_version = 20.0; // Ancient
            }

            // TOTAL SCORE (Weighted)
            // Performance is King (40%), Reliability (30%), Version (20%), Decentralization (10%)
            // This ensures a 16TB node (Perf=100) with 2d uptime (Rel=53) beats a tiny node with high uptime.
            // Power Node: (100*0.4) + (53*0.3) + (100*0.2) + (100*0.1) = 40 + 15.9 + 20 + 10 = ~86
            // Weak Node: (16*0.4) + (100*0.3) + (60*0.2) + (100*0.1) = 6.4 + 30 + 12 + 10 = ~58
            const score = (s_performance * 0.40) + (s_reliability * 0.30) + (s_version * 0.20) + (s_decentralization * 0.10);
